...
The tests cover categorisation, CSV parsing, quoted merchant fields, parser header aliases, multiple currency and debit/credit amount formats, US and day-first dates, invalid parser values, transfer detection, duplicate imports, no-header bank exports, legacy state migration, exact/contains/regex rules, spending summaries, rule creation, PWA static asset wiring, offline module caching, mobile install metadata, and the setup wizard fields for Google/parser/LLM connections.

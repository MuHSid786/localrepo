...
test("handles empty, invalid, and future parser values without throwing", () => {
  assert.equal(parseAmount(""), 0);
  assert.equal(parseAmount("not money"), 0);
  assert.match(toIsoDate("not a date"), /^\d{4}-\d{2}-\d{2}$/);
  assert.equal(parseCsvRows("").length, 0);
});

test("keeps transfer-style transactions out of expense categories", () => {
  const transaction = categoriseTransaction(
    { transactionDate: "2026-05-10", description: "PAYMENT THANK YOU", amount: -250 },
    DEFAULT_RULES
  );
